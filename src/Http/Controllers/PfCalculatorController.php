...
namespace Crmleaf\Payroll\Tools\PfCalculator\Http\Controllers;
...
use Crmleaf\Payroll\Tools\PfCalculator\Http\Requests\PfCalculatorRequest;
use Illuminate\Contracts\View\View;
use Illuminate\Http\JsonResponse;

/**
 * The whole HTTP surface of PF Calculator: one action, one calculator call.
 *
 ...
 * payroll - that all lives in Crmleaf\Payroll\Calculators\PfCalculator - which is why the tool can
 * be embedded, mounted at any prefix, or ignored entirely in favour of calling
 * the calculator yourself.
 */
final class PfCalculatorController
{
    public function __construct(
        private readonly PfCalculator $calculator,
    ) {
    }

    public function __invoke(PfCalculatorRequest $request): JsonResponse|View
    {
    ...
    private function view(PfCalculatorRequest $request, ?CalculationResult $result, ?string $error = null): View
    {
        /** @var \Illuminate\Contracts\Config\Repository $config */
        $config = app('config');

        return view('pf-calculator::pf-calculator', [
            'result' => $result,
            'error' => $error,
            'input' => $request->submitted() ? $request->validated() : [],
            'defaults' => (array) $config->get('pf-calculator.defaults', []),
            'title' => (string) $config->get('pf-calculator.view.title', 'PF Calculator'),
            'action' => $request->url(),
        ]);
    }
}
