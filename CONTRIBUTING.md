...
```bash
git clone https://github.com/crmleaf/pf-calculator.git
cd pf-calculator
composer install
composer test
```
...
requires. `Crmleaf\Payroll\Calculators\PfCalculator` is the class doing the work; everything else
...
Made in the monorepo, against `tools/pf-calculator/`:
