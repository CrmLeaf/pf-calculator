...
<head>
    ...
    <title>{{ $title ?? 'PF Calculator' }}</title>
    <meta name="description" content="EPF employee, employer and EPS shares with the wage ceiling applied.">
    <style>
...
<footer class="crmleaf-tool__colophon">
    ...
    <p>
        Computed by <a href="https://github.com/crmleaf/pf-calculator">crmleaf/pf-calculator</a>,
...
{{--
    ...
    The bundled script binds any element carrying data-crmleaf-tool="pf-calculator",
    computes on submit, and only falls back to posting the form if the browser
    build is not present.
--}}
@if (config('pf-calculator.assets.script', true))
    @if (config('pf-calculator.assets.cdn'))
        <script src="{{ config('pf-calculator.assets.cdn') }}" defer></script>
    @endif
    <script src="{{ asset('vendor/pf-calculator/pf-calculator.js') }}" defer></script>
@endif
