...
namespace Crmleaf\Payroll\Tools\PfCalculator\Tests;

use Crmleaf\Payroll\Calculators\PfCalculator;
use Crmleaf\Payroll\Tools\PfCalculator\PfCalculatorServiceProvider;
 ...
 */
final class PfCalculatorTest extends TestCase
{
    /**
     * Held as a literal rather than as PfCalculator::class so that a calculator
     * which does not exist fails as one readable assertion instead of a fatal
     * error that takes the rest of the file with it.
     */
    private const CALCULATOR = 'Crmleaf\Payroll\Calculators\PfCalculator';
    ...
    {
        return [PfCalculatorServiceProvider::class];
    }
    ...
    protected function defineEnvironment($app): void
    {
        ...
        // pins down.
        $app['config']->set('pf-calculator.route.enabled', true);
    ...
    {
        $calculator = $this->app->make(PfCalculator::class);

        self::assertInstanceOf(PfCalculator::class, $calculator);
        self::assertSame($calculator, $this->app->make(PfCalculator::class), 'The binding should be a singleton.');
    }

    public function test_the_configuration_is_merged(): void
    {
        self::assertSame('pf-calculator', $this->app['config']->get('pf-calculator.route.name'));
        self::assertSame('PF Calculator', $this->app['config']->get('pf-calculator.view.title'));
    }

    public function test_the_route_is_off_until_it_is_asked_for(): void
    {
        /** @var array{route: array{enabled: bool}} $shipped */
        $shipped = require __DIR__.'/../config/pf-calculator.php';
    ...
    {
        $html = Blade::render('<x-crmleaf::pf-calculator />');

        self::assertStringContainsString('data-crmleaf-tool="pf-calculator"', $html);
    ...
    {
        $response = $this->postJson('/tools/pf-calculator', [
            'basic_salary' => 30000,
